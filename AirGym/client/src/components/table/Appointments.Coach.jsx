...
const SubmitLink = ({ sessionId, isOpen, setIsOpen, getSessions }) => {
  ...
  const handleSubmit = async () => {
    ...
    await axios
      ...
      .then(() => {
        successToast("Link submitted successfully");
        setIsOpen(false);
      })
      .catch((err) => {
        ...
        else errorToast("Failed to submit the link");
      })
      .finally(() => {
        setLoading(false);
        setUrl("");
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs font-medium">
            Submit the link for the session to accept the session
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-2">
          ...
          <div className="mt-3 grid gap-4">
            ...
            <Input
              ...
              placeholder="Enter the link for the session"
            />
          </div>
          {/* error message */}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            {session.status === "RESERVED" &&
              (new Date(session.date).getDate() === new Date().getDate() + 1 ||
                (new Date(session.date).getDate() === new Date().getDate() &&
...
export default CoachAppointmentsTable;
